...
if ask1 == 1:
    ...
    A2N = st.text_input("Name of 2nd appliance: ", key="A2N")

    if A2N:
        A2M = st.number_input(f"How many {A2N} are you using?", value=0, step=1, key="A2M")
        ask2 = st.number_input(
            "No more appliances (enter 2): ", value=0, step=1, key="ask2")

        while ask2 > 2 or ask2 < 1:
            ask2 = st.number_input(
                "No more appliances (enter 2): ", value=0, step=1, key="ask2")
